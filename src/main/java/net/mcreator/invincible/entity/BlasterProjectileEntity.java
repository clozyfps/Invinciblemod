...
import net.mcreator.invincible.procedures.BlasterProjectileWhileProjectileFlyingTickProcedure;
...
public class BlasterProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public BlasterProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(InvincibleModEntities.BLASTER_PROJECTILE.get(), world);
	}
	...
	public void tick() {
		super.tick();
		BlasterProjectileWhileProjectileFlyingTickProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
		if (this.inGround)
			this.discard();
	}
	...
	public static BlasterProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		BlasterProjectileEntity entityarrow = new BlasterProjectileEntity(InvincibleModEntities.BLASTER_PROJECTILE.get(), entity, world);
	...
	public static BlasterProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		BlasterProjectileEntity entityarrow = new BlasterProjectileEntity(InvincibleModEntities.BLASTER_PROJECTILE.get(), entity, entity.level());
