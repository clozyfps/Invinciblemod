		...
		if (world.getLevelData().getGameRules().getBoolean(InvincibleModGameRules.SPACEEXIST) == true) {
			if ((entity.level().dimension()) == Level.OVERWORLD) {
				if (entity.getY() >= 350) {
					...
					{
						Entity _ent = entity;
						_ent.teleportTo(x, 50, z);
						if (_ent instanceof ServerPlayer _serverPlayer)
							_serverPlayer.connection.teleport(x, 50, z, _ent.getYRot(), _ent.getXRot());
					}
				}
			} else if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("invincible:space"))) {
				if (entity.getY() <= 25) {
					if (entity instanceof ServerPlayer _player && !_player.level().isClientSide()) {
