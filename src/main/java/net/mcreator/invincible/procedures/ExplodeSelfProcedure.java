...
public class ExplodeSelfProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof ServerLevel _level)
			_level.sendParticles((SimpleParticleType) (InvincibleModParticleTypes.EXPLODE.get()), x, y, z, 100, 5, 5, 5, 0);
		if (world instanceof ServerLevel _level)
			_level.sendParticles((SimpleParticleType) (InvincibleModParticleTypes.LARGE_EXPLODE.get()), x, y, z, 25, 5, 5, 5, 0);
		if (world instanceof ServerLevel _level)
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof Player) {
					if (new Object() {
						...
						entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.PLAYER_EXPLOSION), entity), 50);
					}
				} else {
