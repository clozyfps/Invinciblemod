...
public class SmokeCircleExpandingOnEffectActiveTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		for (int index0 = 0; index0 < 36; index0++) {
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles((SimpleParticleType) (InvincibleModParticleTypes.CUSTOM_SMOKE.get()), x_pos, y_pos, z_pos, 1, 0.1, 0.1, 0.1, 0.1);
		...
		for (int index1 = 0; index1 < 36; index1++) {
			...
			z_pos = z + Math.sin(degree) * zRadius;
			if (world instanceof ServerLevel _level)
				_level.sendParticles((SimpleParticleType) (InvincibleModParticleTypes.CUSTOM_SMOKE.get()), x_pos, y_pos, z_pos, 1, 0.1, 0.1, 0.1, 0);
			degree = degree + Math.toRadians(5);
		}
